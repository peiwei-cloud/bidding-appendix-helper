...
def classify_category(filename: str) -> str:
    ...
    精準化規則：
    - 4_技師執業執照：僅當檔名包含「執業執照」。
    - 5_技師會員證：僅當檔名「同時」包含「水利」與（「會員證」或「公會」）。
      （避免景觀學會會員證等一般協會會員證被誤判為技師會員證）
    - 其餘一般協會/學會證書、電子證書等，一律歸為 3_證照。
    """
    lower = filename.lower()

    if "執業執照" in filename or "执业执照" in filename:
        return "4_技師執業執照"

    has_water = "水利" in filename
    has_member_kw = any(k in filename for k in ["會員證", "会员证", "公會", "公会"])
    if has_water and has_member_kw:
        return "5_技師會員證"
...
def finalize_processing(scan: dict, pending_selection: dict, low_selection: dict,
    ...
    """Phase 2：整合使用者確認後的人員配對結果，執行證照效期檢核、
    AI 履歷結構化萃取，並產出合併附錄 PDF。"""
    ...
    files_by_person = {}
    for fp, person in final_mapping.items():
        category = classify_category(os.path.basename(fp))
    ...
    for i, prow in df_people.iterrows():
        ...
        # 僅對 4_技師執業執照 與 5_技師會員證 進行效期檢核，其餘類別忽略
...
- **人員名單 Excel** 必須包含欄位：`順序`、`姓名`、`部門`、`團隊職務`；
  ...
  未提供也不影響程式運作。
- **三段式雙軌配對機制**：以「姓名」與「英文姓名」為比對清單，忽略大小寫與
  `_` `-` `()` `（）` 空格等符號差異，並容許少量拼字誤差（如 Brain/Brian）：
  - 信心度 ≥ {HIGH_CONFIDENCE_THRESHOLD:.0%}：自動配對，於「已自動配對」清單顯示。
  - 信心度 {MEDIUM_CONFIDENCE_THRESHOLD:.0%}～{HIGH_CONFIDENCE_THRESHOLD:.0%}：需於畫面上確認或修正候選人員。
  - 信心度 < {MEDIUM_CONFIDENCE_THRESHOLD:.0%}：需手動由下拉選單指定人員，或選擇略過。
- **檔案分類**：僅檔名包含「執業執照」→ 技師執業執照；僅當「同時」包含「水利」
  與（「會員證」或「公會」）→ 技師會員證（避免景觀學會等一般會員證誤判）；
  含「投保」或「被保險人」→ 投保證明；含「畢業」或「學歷」→ 學歷證明；
  Word/PDF 檔含「CV／履歷」或為 Word 檔 → CV；其餘一律歸為一般證照。
- **CV 文字擷取**：同時支援 PDF（含 OCR 備援）與 Word（.doc/.docx）格式，
  兩者皆會完整傳送給 Gemini AI 進行 13 欄位結構化萃取。
- **合併 PDF** 會依 Excel 的「順序」欄排列人員，同一人內再依
  CV → 學歷 → 證照 → 執業執照 → 會員證 → 投保證明 排序，並加入書籤方便導覽。
- **證照效期檢核**：僅針對「技師執業執照」與「技師會員證」兩類進行，其餘證照
  類別不判斷過期；支援「自民國X年X月X日至X年X月X日止」、
  「有效期限：民國X年X月X日至X年X月X日」，以及「115年度會員證」
  （自動推算至該年12月31日）等格式；日期一律以當下系統日期動態計算。
