...
LETTER_SPACING_RUN_PATTERN = re.compile(
    r"(?:(?<=^)|(?<=[^A-Za-z0-9]))([A-Za-z0-9](?: [A-Za-z0-9]){2,})(?=[^A-Za-z0-9]|$)"
)


def fix_letter_spacing(text: str) -> str:
    """修復 PDF 逐字抽取造成的英數字母間多餘空白（如「P M P」→「PMP」、
    「A P E C  E n g i n e e r」→「APEC Engineer」）。

    僅收縮「3個以上單一英數字元、以單一半形空白分隔」的連續片段，不影響
    正常單字之間的空白（如「Greenhouse Gas」不會被誤併成「GreenhouseGas」），
    因為正常英文單字本身是多字元的連續字串，不符合「單一字元＋空白」的
    交替樣式。"""
    if not text:
        return ""
    text = LETTER_SPACING_RUN_PATTERN.sub(lambda m: m.group(1).replace(" ", ""), text)
    # 括號緊鄰的空白一併清除，如「師 ( PMP )」→「師(PMP)」
...
# 結束標頭刻意涵蓋「(教育訓練或參與學會)」這種帶括號的段落標題，確保回訓
# 紀錄、參與學會活動等內容不會被誤判為正式證照。
QUALIFICATION_START_KEYWORDS = ["資格及訓練", "專業資格", "資格", "證照"]
QUALIFICATION_END_KEYWORDS = [
    "(教育訓練或參與學會)", "（教育訓練或參與學會）",
    "教育訓練", "工作經歷", "專案經歷", "經歷",
]
...
def extract_qualification_section_text(cv_text: str) -> str:
    """嚴格區塊邊界擷取（Strict Section Extraction）：找到「資格：」「資 格：」
    「證照：」「專業資格：」等起始標頭，一路擷取到下一個「教育訓練：」
    「(教育訓練或參與學會)：」「工作經歷：」「經歷：」「專案經歷：」等段落
    標頭「立刻停止」，只回傳這個區塊「內部」的文字。找不到明確起始標頭則
    回傳空字串（不再退回任何寬鬆比對，避免誤抓其他段落）。"""
    if not cv_text:
        return ""
    ...
    for j in range(start_idx + 1, len(lines)):
        ...
        if QUALIFICATION_END_PATTERN.match(stripped):
            break  # 遇到教育訓練／參與學會／工作經歷等段落標頭，立刻停止
...
def extract_license_candidates_offline(cv_text: str) -> list:
    """100% 僅讀取 CV 內文中「資格」區塊（見 extract_qualification_section_text），
    逐行擷取正式證照/資格全稱候選。**完全不掃描 ZIP 內其他證照檔案的檔名或
    內容**，也不掃描「教育訓練／參與學會」段落下的回訓紀錄，確保資料源頭
    100% 純淨。**不做任何自動縮寫對應**——是否採用（勾選）與對應縮寫代碼，
    完全交由使用者於 Streamlit UI 手動指定。

    規則：
    1. 起始標頭：資格／資 格／證照／專業資格（可有可無冒號、可夾雜空白）。
    2. 結束標頭：教育訓練／(教育訓練或參與學會)／工作經歷／經歷／專案經歷，
       出現即刻停止擷取。
    3. 區塊內「每一行」＝一筆候選（不再依頓號/逗號等分隔符切分同一行）。
    4. 自動清除行首項目符號/編號，並修復 PDF 逐字抽取造成的字母間空白
       （如「P M P」→「PMP」）。
    找不到明確資格區塊標頭時回傳空清單。
    """
    candidates = []
    ...
    for line in section_text.splitlines():
        ...
        line = LICENSE_HEADER_PATTERN.sub("", line).strip()
        line = fix_letter_spacing(line)
        line = clean_license_text(line)
        line = line.strip(" -－、,，;；/")
...
def finalize_processing(scan: dict, pending_selection: dict, low_selection: dict) -> dict:
    ...
    for i, prow in df_people.iterrows():
        ...
        license_candidates = extract_license_candidates_offline(cv_text)
...
def build_license_catalog(license_candidates_by_person: dict, existing_catalog: dict = None) -> dict:
    """全案去重後的證照目錄：text -> {"checked": True, "abbrev": ""}，依人員
    Excel 順序、同一人內出現順序，保留穩定的首次出現順序。這是 Summary
    看板的唯一資料來源——證照是否採用、縮寫代碼皆為「全案層級」設定，
    不再有個人層級的勾選狀態。

    若提供 existing_catalog（例如「套用修正並重新處理」時），會沿用使用者
    已設定過的勾選/縮寫，只為新出現的證照全稱補上預設值。"""
    existing_catalog = existing_catalog or {}
    catalog = {}
    for _name, candidates in license_candidates_by_person.items():
        for text in candidates:
            if text in catalog:
                continue
            if text in existing_catalog:
                catalog[text] = dict(existing_catalog[text])
            else:
                catalog[text] = {"checked": True, "abbrev": ""}
    return catalog


def compute_abbrev_stats(person_licenses: dict, license_catalog: dict) -> dict:
    """統計每個縮寫代碼目前被「已採用（勾選）」證照使用的總人/張數，
    用於 Summary 看板旁的即時統計（如：【技】4人｜【品】3人｜【P】2人）。"""
    license_person_count = {}
    for texts in person_licenses.values():
        for text in texts:
            if license_catalog.get(text, {}).get("checked"):
                license_person_count[text] = license_person_count.get(text, 0) + 1

    stats = {}
    for text, count in license_person_count.items():
        code = (license_catalog.get(text, {}).get("abbrev") or "").strip()
        if code:
            stats[code] = stats.get(code, 0) + count
    return stats


def recompute_licenses_badges(df_staffing: pd.DataFrame, person_licenses: dict,
                               license_catalog: dict) -> pd.DataFrame:
    """依 Summary 看板上「全案層級」的勾選/縮寫狀態，重新計算每位同仁的
    Licenses／Badges 兩欄並寫回 DataFrame，其餘欄位保持不變。

    - Licenses：該同仁持有、且在 Summary 看板中「被勾選採用」的證照全稱，
      頓號分隔。
    - Badges：上述已勾選項目中，「已於 Summary 填寫縮寫」者的代碼，
      半形逗號分隔（同一人重複代碼只列一次）。

    僅更新「存在於 person_licenses 中的人員」列，避免覆蓋使用者手動於
    表格中新增（如協力廠商 Subcontractor）之列的既有內容。"""
    if df_staffing is None or df_staffing.empty:
        return df_staffing
    df = df_staffing.copy()
    for idx, row in df.iterrows():
        name = row.get("Name")
        if name not in person_licenses:
            continue
        texts = person_licenses[name]
        checked_texts = [t for t in texts if license_catalog.get(t, {}).get("checked")]
        df.at[idx, "Licenses"] = "、".join(checked_texts)

        badge_codes = []
        for t in checked_texts:
            code = (license_catalog.get(t, {}).get("abbrev") or "").strip()
...
def run_finalize_and_store(scan: dict) -> None:
    ...
    with st.spinner("處理中，請稍候（含 PDF 轉檔、OCR 效期檢核與離線欄位萃取，可能需數分鐘）..."):
        try:
            ...
            st.session_state.person_licenses = result["license_candidates_by_person"]
            # license_catalog（全案去重後的勾選/縮寫狀態）重新處理時沿用
            # 使用者已設定過的內容，只為新出現的證照全稱補上預設值。
            st.session_state.license_catalog = build_license_catalog(
                result["license_candidates_by_person"],
                existing_catalog=st.session_state.get("license_catalog"),
            )
    ...
    if not excel_file or not zip_file:
        ...
        with st.spinner("掃描檔案並比對人員中，請稍候..."):
            try:
                ...
                st.session_state.person_licenses = {}
                st.session_state.license_catalog = {}
                st.success(
...
with tab2:
    if st.session_state.df_staffing is not None:
        ...
        # ------------------------------------------------------------
        # ③.5 Summary 一站式證照勾選與縮寫指定看板（全案層級，無個人分項勾選）
        # ------------------------------------------------------------
        person_licenses = st.session_state.person_licenses or {}
        if person_licenses:
            st.markdown("#### 🪪 Summary：全案證照彙整與縮寫控制看板")
            st.caption("系統已從每位同仁 CV 的「資格」區塊精準擷取證照/資格全稱"
                       "（不會誤抓教育訓練/工作經歷段落），並彙整全案去重複。下方每種證照"
                       "全稱僅需設定一次「是否採用」與「縮寫代碼」，凡持有該證照的同仁皆會"
                       "自動套用——不再需要逐一到個人卡片勾選。")

            license_catalog = st.session_state.license_catalog or {}
            person_order = st.session_state.scan["names"] if st.session_state.scan else list(person_licenses.keys())

            # 去重後的證照全稱清單，依人員 Excel 順序、同一人內出現順序，
            # 保留穩定的首次出現順序。
            seen = set()
            unique_licenses = []
            for name in person_order:
                for text in person_licenses.get(name, []):
                    if text not in seen:
                        seen.add(text)
                        unique_licenses.append(text)

            # 各證照全稱目前的持有人數（不受勾選狀態影響，僅供參考）
            holder_count = {}
            for texts in person_licenses.values():
                for text in texts:
                    holder_count[text] = holder_count.get(text, 0) + 1

            if not unique_licenses:
                st.caption("目前沒有任何同仁的 CV 擷取到證照/資格候選。")
            else:
                header_c1, header_c2, header_c3 = st.columns([0.6, 3, 1])
                with header_c1:
                    st.caption("採用")
                with header_c2:
                    st.caption("證照/資格全稱（持有人數）")
                with header_c3:
                    st.caption("縮寫")

                for lic_text in unique_licenses:
                    entry = license_catalog.setdefault(lic_text, {"checked": True, "abbrev": ""})
                    c1, c2, c3 = st.columns([0.6, 3, 1])
                    with c1:
                        checked = st.checkbox(
                            "採用", value=entry.get("checked", True),
                            key=f"lic_checked_{lic_text}", label_visibility="collapsed",
                        )
                    with c2:
                        st.write(f"{lic_text}（{holder_count.get(lic_text, 0)} 人）")
                    with c3:
                        abbrev_val = st.text_input(
                            "縮寫", value=entry.get("abbrev", ""),
                            key=f"lic_abbrev_{lic_text}",
                            label_visibility="collapsed", placeholder="縮寫",
                        )
                    license_catalog[lic_text] = {"checked": checked, "abbrev": abbrev_val}

                st.session_state.license_catalog = license_catalog

                stats = compute_abbrev_stats(person_licenses, license_catalog)
                if stats:
                    stats_line = "｜".join(
                ...
                # --- 即時連動：依全案層級的勾選/縮寫狀態，重新計算 Licenses／Badges ---
                st.session_state.df_staffing = recompute_licenses_badges(
                    st.session_state.df_staffing, person_licenses, license_catalog
                )
...
- **🪪 Summary 一站式證照勾選與縮寫控制看板**（tab2，位於配對明細與可編輯
  表格之間）：
  - 證照/資格候選改用**嚴格區塊邊界擷取（Strict Section Extraction）**：僅讀取
    CV 內文中「資格：」「資 格：」「證照：」「專業資格：」等起始標頭之後、
    直到下一個「教育訓練：」「(教育訓練或參與學會)：」「工作經歷：」「經歷：」
    「專案經歷：」標頭「立刻停止」為止的區塊內容；找不到明確起始標頭則該
    同仁清單為空（**不再有任何寬鬆備援比對**）。**完全不掃描 ZIP 內其他
    證照檔案的檔名或內容**，資料來源 100% 純淨僅限 CV「資格」區塊本身，
    確保「教育訓練或參與學會」段落下的回訓紀錄（工地主任回訓、品管回訓等）
    與工作經歷段落的專案說明文字（如「擔任設計技師」「監造技師」）都不會
    被誤抓為證照。區塊內每一行＝一筆候選，並修復 PDF 逐字抽取造成的英數
    字母間空白（如「P M P」→「PMP」、「A P E C  E n g i n e e r」→
    「APEC Engineer」），同時保留括號與連字號等常見符號。
  - **已移除個人卡片逐一勾選機制**：改為單一「Summary 全案證照彙整與縮寫
    控制看板」——系統自動彙整全案所有同仁的證照全稱並去重複，每種全稱僅
    顯示一列（附註持有人數），提供一個「採用」勾選框與一個縮寫輸入框。
    決策層級是「這張證照本身」，不是「某人的這張證照」：只要取消勾選，
    全案所有持有該證照的同仁都不會輸出這張證照；縮寫同理，填寫一次即
    對所有持有者生效。
  - 即時統計：Summary 看板下方即時顯示各縮寫代碼「目前已採用」的總人/張數
    （如「【技】4人｜【品】3人｜【P】2人」）。
  - 即時連動：任何勾選/取消勾選或修改縮寫的操作，都會立即重新計算並寫回
    `Licenses`（該同仁持有且「該證照已被採用」的全稱，頓號分隔）與 `Badges`
    （上述已採用項目中「已填寫縮寫」者的代碼，半形逗號分隔，同一人重複
    代碼只列一次）兩欄，下方表格與下載的 Excel 皆同步反映最新結果。點擊
    「套用修正並重新處理」重新掃描時，已設定過的採用/縮寫狀態會保留，
    只為新出現的證照全稱補上預設值（預設勾選、縮寫留空）。手動於表格新增
    的列（如協力廠商）不受此面板影響，其 `Licenses`／`Badges` 由你直接在
    表格中填寫即可。
