...
3. 以純 Python 正則／規則式邏輯解析每位同仁 CV 全文，離線萃取並產出符合
   Template_Staffing.xlsx 標準格式的 13 個欄位，內容已盡量豐富完整，
...
BADGE_SORT_ORDER = ["水", "土", "環", "保", "結", "景", "技", "P", "品", "安", "採", "博", "碩", "甲", "乙"]
...
def rule_based_badges(cats: dict, badge_relevant_text: str = "") -> set:
    ...
    嚴格防呆：國際/管理類證照（P/採/品/安）與學歷等級（碩/博）、一般技術士等級
    （甲/乙）只要出現關鍵字即可判定；但技師別（水/土/環/保/結/景）必須與
    「技師/執照/證照/執業/檢定」等執業字樣同段落出現才會判定，避免誤抓專案
    名稱（如「景觀工程」專案、「環境影響評估」段落）或純學歷科系（如「土木
    工程學系」）造成誤頒徽章。
    """
    badges = set()
    ...
    # 國際/管理類證照、學歷等級、一般技術士：關鍵字出現即可，誤判風險低
    simple_scope = filename_text + " " + (badge_relevant_text or "")
    simple_keyword_map = {
        "P": ["PMP", "專案管理師", "專案管理國際認證"],
        "採": ["採購專業", "採購人員"],
        "品": ["品質管理", "品管工程師", "品管人員"],
        "安": ["安全衛生", "勞安", "職業安全"],
        "碩": ["碩士"],
    ...
    license_context_keywords = ["技師", "執照", "證照", "证照", "執業", "檢定", "技術士"]
...
BADGE_FULL_NAMES = {
    "水": "水利技師", "土": "土木技師", "環": "環工技師", "保": "水保技師",
    "結": "結構技師", "景": "景觀師", "技": "技師執照", "P": "PMP國際專案管理師",
    "品": "品管工程師", "安": "勞工安全衛生人員", "採": "採購專業人員",
    "博": "博士", "碩": "碩士", "甲": "甲種職業安全主管", "乙": "乙級技術士",
}
...
def finalize_processing(scan: dict, pending_selection: dict, low_selection: dict) -> dict:
    ...
    for i, prow in df_people.iterrows():
        ...
        badges_final = merge_and_sort_badges("", badge_hints)  # Badges 100% 離線判定
