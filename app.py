...
def rule_based_badges(cats: dict, cv_text: str = "") -> set:
    """100% 離線：依檔名與 CV 內文關鍵字組合判定證照代碼，不消耗任何 AI 額度。"""
    badges = set()
...
COMPANY_TITLE_LINE_KEYWORDS = ["現職", "服務單位", "現任職務", "服務機構", "任職公司"]
...
def extract_company_title_offline(cv_text: str):
    """離線正則：搜尋「現職／服務單位／職稱」等關鍵字段落，取出公司與職稱。"""
    if not cv_text:
        return "", ""
    lines = [l.strip() for l in cv_text.splitlines() if l.strip()]
    company, title = "", ""
    ...
    for line in lines:
        if any(kw in line for kw in COMPANY_TITLE_LINE_KEYWORDS):
            ...
            text_part = parts[1].strip() if len(parts) > 1 else line
            tokens = [t for t in re.split(r"[\s/、,，]+", text_part) if t]
            if tokens:
                if not company:
                    company = tokens[0]
                if not title and len(tokens) > 1:
                    title = tokens[1]
...
def extract_expertise_offline(cv_text: str, badges: set) -> str:
    """離線正則：優先從 CV「專長」段落擷取關鍵字；找不到則以已判定的證照代碼
    展開為專長描述，確保欄位不為空。"""
    if cv_text:
        for kw in ["專長", "专长", "專業領域", "专业领域"]:
            idx = cv_text.find(kw)
            if idx != -1:
                segment = cv_text[idx: idx + 120]
...
def build_fallback_bio(name: str, degree: str, company: str, title: str, years) -> str:
    """BioNarrative 保底樣板：無論 AI 是否可用皆確保欄位有完整內容。"""
    degree_part = degree or "相關專業"
    company_part = company or "本公司"
    title_part = title or "工程顧問"
    years_part = years if years else "多"
    return (f"{name}君具備{degree_part}學歷，現任{company_part}{title_part}，"
            f"累積資歷{years_part}年，具備水利與工程專案相關經驗。")


def build_fallback_job_description(role_text: str) -> str:
    """JobDescription 保底樣板：依團隊職務關鍵字對應標準工作內容敘述。"""
    role_text = str(role_text or "")
    for kws, desc in JOB_DESCRIPTION_TEMPLATES:
        if any(kw in role_text for kw in kws):
            return desc
    return "負責所屬工作項目之執行、協調與品質把關，配合專案進度完成各階段任務。"


def build_cv_draft(name: str, role: str, cv_text: str, degree: str, company: str,
                    title: str, years, expertise: str) -> str:
    ...
    project_snippet = "；".join(project_lines[:6]) if project_lines else "（CV中未偵測到明確專案年份/名稱段落）"
...
def gemini_polish_summary(api_keys: list, model_chain: list, name: str, role: str, draft_text: str):
    ...
    prompt = f"""你是專業的標案人員履歷編輯。以下是根據 CV 離線預先萃取出的精簡結構化
摘要草稿，請將其潤飾為更專業、具評審說服力的正式文字，並嚴格以 JSON 格式輸出
（僅輸出 JSON，不要有任何其他文字、不要使用 markdown code block）：
...
def finalize_processing(scan: dict, pending_selection: dict, low_selection: dict,
    ...
    for i, prow in df_people.iterrows():
        ...
        badge_hints = rule_based_badges(cats, cv_text)
        ...
        bio_fallback = build_fallback_bio(name, degree_offline, company_offline, title_offline, years_final)
        job_fallback = build_fallback_job_description(role)
        ...
        if not cv_files:
            ...
            draft = build_cv_draft(name, role, cv_text, degree_offline, company_offline,
                                    title_offline, years_final, expertise_offline)
            try:
                ...
                st.warning(f"⚠️ 「{name}」的 AI 潤飾失敗（已重試/輪詢所有Key仍失敗），"
                           f"已無縫套用離線草稿保底，Excel 欄位依然完整：{e}")
                diag_status = "🟡 資料待補"
                diag_reason = f"AI 潤飾失敗，已套用離線草稿保底：{e}"
            finally:
...
- **離線預填滿 80% 欄位（Phase 1，零 API 消耗、零失敗）**：CV 全文一律完整讀取，
  ...
  - `Badges`：依檔名與 CV 內文關鍵字組合（技/碩/博/品/安/採/乙/甲/景/土/水）。
  - `Expertise`：優先擷取 CV「專長」段落關鍵字，找不到則以已判定的證照代碼
    展開為專長描述。
  - `YearsOfExp`：100% 由 `6_投保證明`「勞保投保年資：X年 X日」離線換算
    （日數 ≥180 進位 +1年，如 6年309日→7年）；無投保證明則為 0。
  - `BioNarrative`／`JobDescription`：先以樣板組成完整保底版本（樣板依團隊
    職務關鍵字自動對應，如「計畫/計劃主持人」→「負責本專案整體規劃、品質
    控管、進度督導與跨單位溝通協調。」）。
- **AI 摘要潤飾（Phase 2，無縫降級）**：僅將 Phase 1 萃取出的「精簡結構化
  草稿」（姓名/學歷/現職/年資/代表性經歷/專長，通常僅數百字）送交 Gemini
  潤飾成正式的 `BioNarrative`（200字內）與 `JobDescription`（50字內），因此
  不需要對原始 CV 全文做任何截斷即可處理任意頁數的長篇履歷（如 17 頁 CV）。
  **若 Gemini 呼叫失敗（429/503/逾時/無 API Key），絕不會填入「請手動補充」
  等佔位字樣，而是直接沿用 Phase 1 的離線保底版本**，確保
  Template_Staffing.xlsx 100% 產出完整內容。
- **多 API Key 輪詢池**：側邊欄可輸入多組 API Key（以逗號分隔，如
  `Key1, Key2`）；遇到 429 配額錯誤會立即無縫切換下一組 Key 重試，不等待；
  503／服務過載才進行指數退避重試；每位人員的 AI 呼叫之間固定間隔 3.5 秒，
  降低整體觸發 429 的機率。
