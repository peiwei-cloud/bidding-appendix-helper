...
def build_cv_draft(name: str, role: str, group_name: str, cv_text: str, degree: str,
                    company: str, title: str, years, expertise: str, project_snippet: str,
                    work_content: str = "") -> str:
    ...
    return (
        ...
        f"CV主要工作/職務內容：{work_content or '（CV中未偵測到明確工作內容段落）'}"
    )
...
BATCH_SIZE = 4  # 微批次打包處理：每 4 人合併為一次 Gemini 請求


def gemini_polish_batch(api_keys: list, model_chain: list, batch_items: list):
    """微批次打包處理（Micro-Batching）：一次請求同時潤飾最多 BATCH_SIZE
    （預設 4）位人員的 BioNarrative／JobDescription，大幅降低 API 呼叫次數，
    從根本緩解 429 限流問題。

    batch_items：[{"name":..., "role":..., "group_name":..., "draft":...}, ...]
    回傳：dict，key 為姓名，value 為 {"BioNarrative":..., "JobDescription":...}。
    整批（所有 Key × 所有模型皆試過）仍失敗時拋出例外，由呼叫端將「這一批」
    全數降級為離線保底生成，其餘批次不受影響（顆粒化容錯）。
    ...
    names_list = "、".join(item["name"] for item in batch_items)

    prompt = f"""你是專業的標案人員履歷編輯，比照政府標案「服務建議書」附錄人員簡歷的
撰寫風格。以下提供 {len(batch_items)} 位人員（{names_list}）各自的離線預先萃取精簡
結構化摘要草稿，請「逐一」將每一位潤飾為更專業、具評審說服力的正式文字，並嚴格以
JSON 格式輸出（僅輸出 JSON，不要有任何其他文字、不要使用 markdown code block）：

{{
  "results": [
    {{
      "Name": "人員姓名（須與輸入完全一致，用於比對）",
      "BioNarrative": "200字以內，須採四段式結構撰寫（文字需連貫，不要條列）：①學歷與現職（含最高學歷、現職公司與職稱）②代表性經歷（從該人員草稿的代表性經歷中，精準點出1~2項重點專案名稱）③核心專長（涵蓋相關工程領域關鍵字）④履約效益（強調團隊協調與品質控管能力，能確保本案高品質履約）",
      "JobDescription": "50字以內，須同時依據此人擔任的團隊職務、所屬組別，以及草稿中『CV主要工作/職務內容』，具體歸納其在本專案中擬任的工作內容，每位人員不可套用完全相同的罐頭句子"
    }}
  ]
}}

results 陣列須包含全部 {len(batch_items)} 位人員，且順序、姓名須與下方輸入一致。
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    for r in results:
                        ...
                        if nm:
                            out[nm] = {
                                "BioNarrative": r.get("BioNarrative", ""),
                                "JobDescription": r.get("JobDescription", ""),
                            }
                    return out, model, key_idx
...
def finalize_processing(scan: dict, pending_selection: dict, low_selection: dict,
    ...
    # ---------- Pass 1：對所有人先完成 Phase 1 100% 離線預填 ----------
    person_records = []  # 依 Excel 順序保存每個人的離線資料與草稿
    no_cv_people = []

    prep_progress = st.progress(0.0, text="正在進行離線欄位預填...")
    ...
    for i, prow in df_people.iterrows():
        ...
        prep_progress.progress(i / total, text=f"離線預填：{name}")
        ...
        job_fallback = build_fallback_job_description(role, group, work_content_offline)
        ...
        if needs_ai:
            draft = build_cv_draft(name, role, group, cv_text, degree_offline, company_offline,
                                    ...
                                    work_content_offline)

        person_records.append({
            "name": name, "role": role, "group": group,
            "final_data": final_data, "years_final": years_final,
            "badge_hints": badge_hints, "needs_ai": needs_ai, "draft": draft,
            "diag_status": diag_status, "diag_reason": diag_reason,
            "files_by_category": files_by_category_named,
        })

    prep_progress.progress(1.0, text="離線預填完成")
    prep_progress.empty()

    # ---------- Pass 2：微批次打包處理（Micro-Batching），每 BATCH_SIZE 人一批 ----------
    ai_candidates = [r for r in person_records if r["needs_ai"]]
    batches = [ai_candidates[i:i + BATCH_SIZE] for i in range(0, len(ai_candidates), BATCH_SIZE)]

    if batches:
        batch_progress = st.progress(0.0, text="開始進行 AI 摘要潤飾（微批次打包處理）...")
        total_batches = len(batches)
        for b_idx, batch in enumerate(batches, start=1):
            batch_names = "、".join(r["name"] for r in batch)
            batch_progress.progress(
                (b_idx - 1) / total_batches,
                text=f"AI 摘要潤飾（批次 {b_idx}/{total_batches}）：{batch_names}",
            )
            batch_items = [
                {"name": r["name"], "role": r["role"], "group_name": r["group"], "draft": r["draft"]}
                for r in batch
            ]
            try:
                batch_results, _used_model, _key_idx = gemini_polish_batch(
                    api_keys, model_chain, batch_items
                )
                for r in batch:
                    result = batch_results.get(r["name"])
                    if result and (result.get("BioNarrative") or result.get("JobDescription")):
                        if result.get("BioNarrative"):
                            r["final_data"]["BioNarrative"] = result["BioNarrative"]
                        if result.get("JobDescription"):
                            r["final_data"]["JobDescription"] = result["JobDescription"]
                        r["diag_status"] = "🟢 完整解析"
                    else:
                        r["diag_status"] = "🟡 資料待補"
                        r["diag_reason"] = "AI 批次結果未包含此人員，已套用離線草稿保底"
            except Exception as e:
                short_reason = summarize_ai_error(e)
                st.warning(f"⚠️ 第 {b_idx} 批（{batch_names}）AI 潤飾暫時無法使用（{short_reason}），"
                           "已無縫切換至高品質離線精準生成模式，該批人員 Excel 欄位依然完整；"
                           "其餘批次不受影響。")
                for r in batch:
                    r["diag_status"] = "🟡 資料待補"
                    r["diag_reason"] = f"AI 批次潤飾暫時無法使用（{short_reason}），已套用離線草稿保底"

            # 批次間加入微小延遲，降低 429 (Rate Limit) 風險
            time.sleep(1.2 + random.uniform(0, 0.3))

        batch_progress.progress(1.0, text="AI 摘要潤飾完成")
    ...
    for r in person_records:
        diagnostics[r["name"]] = {
            "status": r["diag_status"],
            "reason": r["diag_reason"],
            "files_by_category": r["files_by_category"],
        }

        layer = determine_layer(r["role"])
        badges_final = merge_and_sort_badges("", r["badge_hints"])  # Badges 已 100% 離線判定
        group = r["group"]
        final_data = r["final_data"]
        ...
        staffing_rows.append({
            ...
            "Badges": badges_final,
            "PhotoName": f"{r['name']}.jpg",
            "YearsOfExp": r["years_final"],
...
- **離線預填滿 80% 欄位（Phase 1，零 API 消耗、零失敗）**：CV 全文一律完整讀取，
  ...
  - `YearsOfExp`：優先由 `6_投保證明`「勞保投保年資：X年 X日」離線換算
    ...
    偵測到即用）；兩者皆無資料則預設為 0。全程不消耗 AI 額度。
  - `BioNarrative`／`JobDescription`：先以樣板組成完整保底版本。
    ...
    …」），不會所有人套用同一句話。`BioNarrative` 採四段式標案範本風格：
    ①學歷與現職 ②代表性經歷（CV 中 1~2 項重點專案） ③核心專長 ④履約效益。
  - **CV 主要工作/職務內容**：從 CV「經歷／工作內容／專案職責」等段落，擷取
    2~3 句含「負責/辦理/執行/規劃…」等動詞的實際工作內容，同時供離線
    `JobDescription` 樣板與送交 AI 的草稿使用。
- **微批次打包處理（Micro-Batching，核心限流對策）**：全體人員依 Excel 順序
  每 `BATCH_SIZE = 4` 人切分為一組，**一次 API 請求同時潤飾 4 人**的
  `BioNarrative`／`JobDescription`，大幅降低請求次數；批次之間固定間隔約
  1.2 秒微小延遲。若某一批（4人）呼叫失敗，**僅該批**自動降級為離線草稿
  保底，其餘批次的 AI 潤飾成果不受影響（顆粒化容錯）。送交 AI 的是 Phase 1
  萃取出的精簡結構化草稿（非原始 CV 全文），因此不需要對 CV 全文做任何
  截斷即可處理任意頁數的長篇履歷（如 17 頁 CV）。**若批次呼叫失敗
...
- **Gemini 模型鏈與限流防護**：預設候選模型為
  ...
  的虛構模型 ID，避免無效試錯觸發連鎖 429/404）；503／服務過載時進行指數
  退避重試。
