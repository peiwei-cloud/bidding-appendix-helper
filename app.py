...
def gemini_extract_person(client, model_chain, name, role, group, cv_text, badge_hints):
    """呼叫 Gemini 進行 CV 結構化萃取。**僅**用於 1_CV 履歷內容——證照效期與
    投保年資一律以離線 Regex/OCR 解析，完全不消耗 AI 額度。

    為避免多頁長篇 CV（如 17 頁履歷）撐爆 TPM 導致 429，CV 全文一律截斷至
    前 6000 字再送出。"""
    truncated_cv = cv_text[:6000]
...
CV全文如下（若原文超過6000字，此處僅為截斷後的前段內容）：
---
{truncated_cv}
---
"""
    ...
    for model in model_chain:
        max_attempts = 3  # 429 情境下：1次初始嘗試 + 最多2次重試
            ...
            try:
                ...
                if is_rate_limit:
                    ...
                    if attempt < max_attempts - 1:
                        time.sleep(10)  # 429：固定等待 10 秒後重試
                        continue
                    break  # 重試 2 次仍失敗，換下一個候選模型
...
def finalize_processing(scan: dict, pending_selection: dict, low_selection: dict,
    ...
    for i, prow in df_people.iterrows():
        ...
        if not cv_files:
            ...
            diag_reason = "ZIP 內未比對到任何 CV 檔案"
        elif not cv_text.strip():
            diag_status = "🟡 資料待補"
            diag_reason = "CV 檔案無法擷取到文字內容（可能為純掃描影像，OCR 失敗）"
            st.warning(f"⚠️ 「{name}」的 CV 檔案無法擷取到文字內容（可能為純掃描影像），"
                       "請確認 OCR 環境或改由手動填寫。")
        elif not api_key:
            diag_status = "🟡 資料待補"
            diag_reason = "未提供 Gemini API Key，AI 欄位未執行"
        elif not client:
            diag_status = "🟡 資料待補"
            diag_reason = "Gemini Client 初始化失敗，請確認 API Key 是否正確"
        else:
            try:
                ...
                st.warning(f"⚠️ 「{name}」的 AI 履歷解析失敗（已重試仍失敗），"
                           f"相關欄位將填入「[請手動補充]」以利後續人工處理：{e}")
                ai_data = {}
                diag_status = "🟡 資料待補"
                diag_reason = f"AI 解析失敗：{e}"
            finally:
        ...
        # 投保年資雙軌備援：離線解析（優先權1）> Gemini AI 由 CV 推算（優先權2）
        offline_years = insurance_years_by_person.get(name)
        if offline_years is not None:
            years_final = offline_years
        else:
            years_final = ai_data.get("YearsOfExp", 0) or 0

        ai_failed = bool(cv_files) and cv_text.strip() and client and not ai_data
        placeholder = "[請手動補充]"
        ...
        staffing_rows.append({
            ...
            "Name": name,
            "Title": (ai_data.get("Title") or (placeholder if ai_failed else "")),
            "Company": (ai_data.get("Company") or (placeholder if ai_failed else "")),
            "Badges": badges_final,
            "PhotoName": f"{name}.jpg",
            "YearsOfExp": years_final,
            "Degree": (ai_data.get("Degree") or (placeholder if ai_failed else "")),
            "JobDescription": (ai_data.get("JobDescription") or (placeholder if ai_failed else "")),
            "Expertise": (ai_data.get("Expertise") or (placeholder if ai_failed else "")),
            "BioNarrative": ai_data.get("BioNarrative") or (placeholder if ai_failed else "[資料待補]"),
        })
...
def run_finalize_and_store(scan: dict, api_key: str, model_name: str) -> None:
...
if scan is not None:
    ...
    if process_btn:
        run_finalize_and_store(scan, api_key, selected_model)
else:
...
with tab2:
    if st.session_state.df_staffing is not None:
        ...
        if missing_people:
            st.error(
                "以下人員缺乏 CV 履歷檔，表格欄位已留空，請於下方表格手動補充或重新上傳檔案：\n\n"
                + "、".join(missing_people)
            )
        ...
        if warning_people:
            ...
            st.warning("以下人員的 AI 解析結果可能不完整，請檢查原因並視需要手動補充：\n\n" + lines)
        ...
        if st.session_state.unmatched_files:
            with st.expander(f"⚠️ 有 {len(st.session_state.unmatched_files)} 個檔案未成功比對到任何人員，"
                ...
                if st.button("🔄 套用修正並重新處理", key="tab2_apply_fix"):
                    run_finalize_and_store(st.session_state.scan, api_key, selected_model)
                    st.rerun()
...
- **證照效期檢核**：僅針對「技師執業執照」與「技師會員證」兩類進行，其餘證照
  ...
  以當下系統日期（`date.today()`）動態計算。**證照與投保證明的解析全程為離線
  Regex／OCR，完全不呼叫 Gemini API。**
- **投保年資（YearsOfExp）雙軌備援**：優先從投保證明內文（如「勞保投保年資：
  6年 309日（截至115/07/22止）」）以離線 Regex 精準換算年資（日數 ≥180 進位
  +1年，如6年309日→7年），完全不消耗 AI 額度；僅當同仁無投保證明或無法解析
  時，才改由 Gemini AI 從 CV 內容推算。
- **Gemini API 呼叫保護（防 429）**：AI 僅用於 1_CV 履歷結構化萃取；CV 全文一律
  截斷至前 6000 字再送出，避免長篇履歷（如 17 頁 CV）撐爆 TPM；每位人員的 AI
  呼叫之間固定間隔 3.5 秒；遇到 429 錯誤會等待 10 秒後重試，最多重試 2 次，
  仍失敗則該人員相關欄位填入「[請手動補充]」並繼續處理下一位，不會中斷整體
  流程。
