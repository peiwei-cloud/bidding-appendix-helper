...
with tab2:
    if st.session_state.df_staffing is not None:
        ...
        if license_state:
            ...
            # --- ② 下方：Summary 看板，動態過濾去重——只顯示「目前有被勾選」的全稱 ---
            # 依上方最新勾選狀態即時彙整，未被任何同仁勾選的證照一律隱藏不顯示。
            unique_licenses = get_unique_checked_licenses(license_state, person_order)
            abbrev_map = st.session_state.license_abbrev_map or {}

            st.markdown("##### 📌 Summary：證照/資格縮寫指定（僅列出已勾選項目，去重後）")
            if not unique_licenses:
                st.caption("目前沒有任何已勾選的證照/資格，請先於上方展開人員面板勾選。")
            else:
...
- **🪪 證照/資格勾選 ＋ Summary 集中輸入縮寫面板**（tab2，位於配對明細與可編輯
  ...
  - 介面順序為**先個人勾選、後 Summary 縮寫**：置頂為「👤 個人證照/資格勾選」
    （`st.expander` 依人員摺疊呈現，**僅提供勾選框**，預設全部勾選，不再逐項
    填縮寫）；下方為「📌 Summary：證照/資格縮寫指定」，**動態過濾且去重**——
    只列出「目前有被勾選」的證照全稱，取消勾選的證照會立刻從 Summary 隱藏
    （縮寫仍保留在背景，重新勾選會自動帶回原本填過的縮寫）。
  - 縮寫代碼於 Summary 看板**集中輸入**：去重後的每種全稱只出現一個縮寫
    輸入框——多人持有同一張證照，縮寫只需填寫一次、全案自動套用；旁邊即時
    顯示各縮寫代碼的統計（如「【技】4人｜【品】3人｜【P】2人」）。
