...
def extract_license_candidates_offline(cv_text: str, cats: dict, name: str = "") -> list:
    """100% 離線：從 CV「學歷／證照／技師／資格」相關段落，擷取同仁擁有的所有
    「證照/資格全稱」候選清單。**不做任何自動縮寫對應**——是否採用（勾選）與
    對應縮寫代碼，完全交由使用者於 Streamlit UI 手動指定。

    來源：
    1. CV 內文中，經 extract_badge_relevant_text() 篩選出的資格相關段落，
       依常見分隔符（、,，;；/）切分成個別候選項目。
    2. 已分類為「4_技師執業執照」／「5_技師會員證」的證照檔名，各自視為
    ...
    for line in badge_relevant_text.splitlines():
...
def build_default_license_state(license_candidates_by_person: dict) -> dict:
    """依離線擷取到的證照/資格候選清單，建立預設「全部勾選、縮寫留空」的
    UI 狀態結構，供 tab2 的勾選面板讀取與編輯。"""
    state = {}
    for name, candidates in license_candidates_by_person.items():
        state[name] = [
            {"text": c, "checked": True, "abbrev": ""} for c in candidates
        ]
    return state


def recompute_licenses_badges(df_staffing: pd.DataFrame, license_state: dict) -> pd.DataFrame:
    """依目前 UI 上的勾選/縮寫狀態，重新計算 Licenses／Badges 兩欄並寫回
    DataFrame，其餘欄位保持不變。

    - Licenses：輸出「已勾選」的證照/資格全稱，頓號分隔。
    - Badges：僅輸出「已勾選且已手動填寫縮寫」的代碼，半形逗號分隔。
    ...
    for idx, row in df.iterrows():
        ...
        badge_codes = [it["abbrev"].strip() for it in checked_items if it.get("abbrev", "").strip()]
...
with tab2:
    if st.session_state.df_staffing is not None:
        ...
        if license_state:
            ...
            st.caption("系統已離線擷取每位同仁的證照/資格全稱候選清單（預設全部勾選、"
                       "縮寫留空，不套用任何自動縮寫字典）。請逐一勾選是否採用，並視需要"
                       "手動輸入/修改縮寫代碼（如：技、品、P、碳、安、綠…）。下方 Summary "
                       "看板與表格的 Licenses／Badges 欄位會即時連動更新。")

            # --- 頂部 Summary 彙整看板 ---
            summary_counter = {}  # abbrev -> {"count": n, "sample": 第一筆全稱}
            for _name, items in license_state.items():
                for item in items:
                    if item.get("checked") and item.get("abbrev", "").strip():
                        code = item["abbrev"].strip()
                        entry = summary_counter.setdefault(code, {"count": 0, "sample": item["text"]})
                        entry["count"] += 1

            if summary_counter:
                summary_line = "｜".join(
                    f"【{code}】{info['sample'][:8]} {info['count']} 人/張"
                    for code, info in sorted(summary_counter.items(), key=lambda x: -x[1]["count"])
                )
                st.info(f"📌 目前彙整：{summary_line}")
            else:
                st.caption("尚未指定任何縮寫代碼，Badges 欄位目前皆為空白。")

            # --- 每位人員的勾選／縮寫編輯區（依 Excel 順序摺疊呈現）---
                    ...
                    for idx, item in enumerate(items):
                        ...
                        with c2:
                            abbrev = st.text_input(
                                "縮寫", value=item.get("abbrev", ""),
                                key=f"lic_abbrev_{name}_{idx}",
            ...
            # --- 即時連動：依目前勾選/縮寫狀態，重新計算 Licenses／Badges 欄位 ---
            st.session_state.df_staffing = recompute_licenses_badges(
                st.session_state.df_staffing, license_state
            )
...
- **🪪 證照/資格勾選與縮寫指定面板**（tab2，位於配對明細與可編輯表格之間）：
  - 每位同仁以 `st.expander` 摺疊呈現其離線擷取到的所有證照/資格全稱候選，
    每項提供勾選框（預設全部勾選）與可自由編輯的縮寫輸入框（預設留空，可
    輸入任意代碼如「技」「品」「P」「碳」「安」「綠」…，無任何預設對照表）。
  - 頂部 Summary 看板即時彙整「目前所有同仁已勾選且已填寫縮寫」的代碼種類、
    對應樣本名稱與總人/張數（如「【技】水利技師執照 4 人/張｜【P】國際專案
    管理師 2 人/張」）。
  - 即時連動：任何勾選/取消勾選或修改縮寫的操作，都會立即重新計算並寫回
    `Licenses`（僅列出已勾選項目的全稱，頓號分隔）與 `Badges`（僅列出已勾選
    且已填寫縮寫的代碼，半形逗號分隔）兩欄，下方表格與下載的 Excel 皆同步
    反映最新結果。手動於表格新增的列（如協力廠商）不受此面板影響，其
    `Licenses`／`Badges` 由你直接在表格中填寫即可。
